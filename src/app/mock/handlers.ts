...
export function getHandlers(base: string) {
  return [
    ...
    http.get(`${base}/api/credentials/export`, ({ request }) => {
    ...
    http.get(`${base}/api/credentials/:id`, ({ request, params }) => {
      const userId = getUserFromRequest(request);
      if (!userId) return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 });
    ...
    http.get(`${base}/api/credentials`, ({ request }) => {
      const userId = getUserFromRequest(request);
      if (!userId) return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 });
    ...
    http.post(`${base}/api/credentials`, async ({ request }) => {
      const userId = getUserFromRequest(request);
      if (!userId) return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 });
    ...
    http.patch(`${base}/api/credentials/:id`, async ({ request, params }) => {
      const userId = getUserFromRequest(request);
      if (!userId) return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 });
    ...
    http.delete(`${base}/api/credentials/:id`, ({ request, params }) => {
      const userId = getUserFromRequest(request);
      if (!userId) return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 });
